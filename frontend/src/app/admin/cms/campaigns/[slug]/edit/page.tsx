...
export default function EditCampaignPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    adminApi.cmsCampaigns.get(slug).then((r) => reset({ ...r.data, current_image: r.data.image, image: undefined }));
  }, [slug, reset]);
  ...
  async function onSubmit(values: any) {
    try {
      ...
      Object.entries(values).forEach(([k, v]) => {
        if (k === 'current_image' || k === 'progress_percent' || k === 'created_at' || k === 'donations') return;
      ...
      await adminApi.cmsCampaigns.update(slug, fd, true);
