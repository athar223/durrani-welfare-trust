...
export default function EditDailyExpensePage() {
  ...
  useEffect(() => {
    if (!id) return;
    adminApi.dailyExpenses.get(Number(id)).then((r) => reset(r.data));
  }, [id, reset]);

  async function onSubmit(values: any) {
    try {
      await adminApi.dailyExpenses.update(Number(id), values);
  ...
  return (
    <AdminLayout title="Edit Daily Expense">
